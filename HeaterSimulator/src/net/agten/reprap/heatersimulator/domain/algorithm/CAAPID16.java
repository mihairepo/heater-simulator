 ...
 * 
 */
public class CAAPID16 implements ControllerAlgorithm {
	/**
	 ...
	 * 
	 * @param pGain
	 *            the gain for proportional component of the algorithm
	 * @param iGain
	 *            the gain for integral component of the algorithm
	 * @param dGain
	 *            the gain for derivative component of the algorithm
	 * @param iWindupGuard
	 *            the maximum absolute value of the integral component of the
	 *            algorithm
	 * @param outputDivisor
	 *            integer by which to divide the algorithm's output
	 */
	public CAAPID16(short pGain, short iGain, short dGain, short outputDivisor) {
		if (outputDivisor == 0) {
	 ...
	 * 
	 * @param curAdc
	 *            a 10-bit ADC value representing the current reading
	...
	public short nextValue(short curAdc) {
		short error = (short) (this.targetAdc - curAdc);

		// Calculate the proportional term. This term uses information from the
		// now.
		short pTerm = (short) (pGain * error);

		// Calculate the differential term. This term uses the difference
		// between
		// the current ADC value and the last ADC value to determine the speed
		// by which the ADC value is changing.
		short dTerm = (short) (dGain * (short) (lastAdc - curAdc));
		this.lastAdc = curAdc;

		short result = (short) ((short) ((pTerm + dTerm)) / outputDivisor);
		short iTerm = 0;
		...
		if (result < MAX_RESULT) {
			...
			// Calculate the integral term. This term uses information from the
			// past.
			iTerm = (short) (iGain * iState);
		}

		// Calculate the sum of each term, divided by the output divisor
		result += iTerm / outputDivisor;
		// Cap the result to [0,MAX_RESULT]
		result = (short) Math.max(Math.min(result, MAX_RESULT), 0);
